...
/*
Errore in caso tutte le aziende abbiano affidabilità = 0 oppure se c è un pareggio tra più aziende con affidabilità maggiore
-- Vista 3: per mostrare l'azienda con il valore più alto di affidabilità
CREATE OR REPLACE VIEW AziendaAffidabilitàMaggiore AS
SELECT
A.RagioneSociale,
A.Nome AS NomeAzienda,
A.Settore,
...
T.NumTotali,
(
P.NumApprovazioni / T.NumTotali
) * 100 AS Percentuale_Affidabilita
FROM Azienda A
-- Uniamo con la tabella dei bilanci totali 
JOIN (
SELECT BilancioAz, COUNT(*) as NumTotali
FROM Revisione
WHERE
Esito IS NOT NULL
GROUP BY
BilancioAz
) T ON A.RagioneSociale = T.BilancioAz
-- Uniamo con la tabella delle sole approvazioni
JOIN (
SELECT BilancioAz, COUNT(*) as NumApprovazioni
FROM Revisione
WHERE
Esito = 'Approvazione'
GROUP BY
BilancioAz
) P ON A.RagioneSociale = P.BilancioAz
-- prendiamo solo il primo (quella più affidabile)
